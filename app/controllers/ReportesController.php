    ...
    public function ReportesEmpleados($request, $response, $args){
        ...
        if(!isset($estadistica)){
            ...
            if(!$estadistica && (!$this->validateDate($fecha_inicio) || !$this->validateDate($fecha_fin))){
                ...
                $pdf->AddPage();
                $pdf->Cell(150,10,'Los dias y horarios que se Ingresaron al sistema: ' ,0,1);
                $pdf->MultiCell(150,10, Usuario::ingresosSistema($fecha_inicio, $fecha_fin));
                $pdf->Cell(150,10,'Cantidad de operaciones de todos por sector: ' ,0,1);
                $pdf->MultiCell(150,10, Usuario::operacionesPorSector($fecha_inicio, $fecha_fin));
                $pdf->Cell(150,10,'Cantidad de operaciones de todos por sector, listada por cada empleado: ' ,0,1);
                $pdf->MultiCell(150,10, Usuario::operacionesPorSectorYPorUsuario($fecha_inicio, $fecha_fin));
                $pdf->Cell(150,10,'Cantidad de operaciones de cada uno por separado: ' ,0,1);
    ...
    public function ReportesPedidos($request, $response, $args){
        ...
        if(!isset($estadistica)){
            ...
            if(!$estadistica && (!$this->validateDate($fecha_inicio) || !$this->validateDate($fecha_fin))){
                ...
                $pdf->AddPage();
                $pdf->Cell(150,10,'Lo que mas se vendio: ' . Pedido::masVendido($fecha_inicio, $fecha_fin) ,0,1);
                $pdf->Cell(150,10,'Lo que menos se vendio: ' . Pedido::menosVendido($fecha_inicio, $fecha_fin) ,0,1);
                $pdf->MultiCell(150,10,'Los que no se entregaron en el tiempo estipulado: ' . Pedido::entregadosFueraTiempo($fecha_inicio, $fecha_fin) ,0,1);
                $pdf->MultiCell(150,10,'Los cancelados: ' . Pedido::cancelados($fecha_inicio, $fecha_fin) ,0,1);
                $pdf->Output();
                ob_end_flush();
                $payload = json_encode(array("mensaje" => "Reporte pedidos descargado"));
                $response->getBody()->write($payload);
                return $response->withHeader('Content-Type', 'application/pdf');
            }
        }
        ...
        */
        ...
        if(!isset($estadistica)){
            ...
            if(!$estadistica && (!$this->validateDate($fecha_inicio) || !$this->validateDate($fecha_fin))){
                ...
                $payload = json_encode(array("mensaje" => "Reporte mesas descargado"));
                $response->getBody()->write($payload);
                return $response->withHeader('Content-Type', 'application/pdf');
            }
        }
